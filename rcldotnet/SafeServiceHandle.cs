...
{
    /// <summary>
    /// Safe handle representing a rcl_service_t
    /// </summary>
    /// <summary>
    /// Since we need to delete the service handle before the node is released we need to actually hold a
    /// pointer to a rcl_service_t unmanaged structure whose destructor decrements a refCount. Only when
    /// the node refCount is 0 it is deleted. This way, we loose a race in the critical finalization
    /// of the service handle and node handle. This also applies to publisher, subscription and client handles,
    /// which point to a rcl_publisher_t, rcl_subscription_t or rcl_client_t.
    /// </summary>
    ...
    { 
        ...
        //
        // Needed to change the exact order of statements in ReleaseOrder() as rcl_service_fini()
        ...
        protected override bool ReleaseHandle()
        {
            ...
            if (parent != null)
            {
                ...
                RCLRet ret = NodeDelegates.native_rcl_destroy_service_handle(handle, parent);
