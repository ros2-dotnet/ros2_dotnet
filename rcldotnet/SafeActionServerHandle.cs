...
{
    /// <summary>
    /// Safe handle representing a rcl_action_server_t
    /// </summary>
    /// <summary>
    /// Since we need to delete the action client handle before the node is released we need to actually hold a
    /// pointer to a rcl_action_server_t unmanaged structure whose destructor decrements a refCount. Only when
    ...
    /// </summary>
    internal sealed class SafeActionServerHandle : SafeHandleZeroOrMinusOneIsInvalid
    {
        ...
        protected override bool ReleaseHandle()
        {
            ...
            if (parent != null)
            {
                ...
                RCLRet ret = NodeDelegates.native_rcl_action_destroy_server_handle(handle, parent);
